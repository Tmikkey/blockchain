...
export default {
  ...
  created() {
    /** Array of props of `BBreakpoint` components*/
    this._breakpoints = []
  },
  ...
  methods: {
    /**
     * Collect `BBreakpoint`'s data from default slot.
     */
    prepareBreakpoints() {
      ...
      children.forEach((_child) => {
        if (_child.$options._componentTag === 'b-breakpoint') {
          breakpoint = {
            ..._child.$props,
          }
    ...
    onBreakpointChange(_event) {
      ...
      if (breakpoint.length === 1) {
        ...
        this.emitCustomEvent('b-breakpoint-change', data)
      }
    },
  },
}
